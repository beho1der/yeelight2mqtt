...
for target in "${TARGETS[@]}"; do
    ...
    #env GOOS=linux GOARCH=$ARCH GOARM=$ARM $SOFTFLOAT CGO_ENABLED=$CGO tinygo build -no-debug -o ${PKG_DIR}/data/usr/bin/${PKG_NAME}
    env GOOS=linux GOARCH=$ARCH GOARM=$ARM $SOFTFLOAT CGO_ENABLED=$CGO go build -ldflags="-s -w" -o ${PKG_DIR}/data/usr/bin/${PKG_NAME}
